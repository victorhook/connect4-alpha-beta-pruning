...
from gym_connect_four.render import render_board
...
class ConnectFourEnv(gym.Env):
    ...
    def __init__(self, board_shape=(6, 7), window_width=512, window_height=512,
        ...
        if env is not None:
            ...
            self._board = env.board
            self.player_color = env.player_color
        else:
            self.current_player = 1
            self._board = np.zeros(self.board_shape, dtype=int)
            self.player_color = 1

        self.last_reward = self.DEF_REWARD
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, dict]:
        ...
        reward = step_result.get_reward(self.current_player)
        done = step_result.is_done()
        self.last_reward = reward
        return self._board.copy(), reward, done, {}
    ...
    def _step(self, action: int) -> StepResult:
        ...
        for index in list(reversed(range(self.board_shape[0]))):
            if self._board[index][action] == 0:
                self._board[index][action] = self.current_player
                break

        # Check if board is completely filled
        if np.count_nonzero(self._board[0]) == self.board_shape[1]:
            result = ResultType.DRAW
        else:
            # Check win condition
            if self.is_win_state():
                result = ResultType.WIN1 if self.current_player == 1 else ResultType.WIN2
    ...
    def _update_board_render(self) -> np.ndarray:
        return render_board(self._board,
    ...
    def is_win_state(self) -> bool:
        ...
        for i in range(self.board_shape[0]):
            for j in range(self.board_shape[1] - 3):
                value = sum(self._board[i][j:j + 4])
                if abs(value) == 4:
                    return True

        # Test columns on transpose array
        reversed_board = [list(i) for i in zip(*self._board)]
        ...
        for i in range(self.board_shape[0] - 3):
            for j in range(self.board_shape[1] - 3):
                value = 0
                for k in range(4):
                    value += self._board[i + k][j + k]
                    if abs(value) == 4:
                        return True

        reversed_board = np.fliplr(self._board)
